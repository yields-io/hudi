...
public class HoodieCommitArchiveLog {
  ...
  private boolean deleteArchivedInstants(List<HoodieInstant> archivedInstants) throws IOException {
    ...
    Optional<HoodieInstant> youngestCommitted =
    ...
    if (youngestCommitted.isPresent()) {
      success &= deleteAllInstantsOlderorEqualsInAuxMetaFolder(youngestCommitted.get());
    }
    return success;
  }
   ...
   */
  private boolean deleteAllInstantsOlderorEqualsInAuxMetaFolder(HoodieInstant instant)
      throws IOException {
    List<HoodieInstant> instants =
        HoodieTableMetaClient.scanHoodieInstantsFromFileSystem(metaClient.getFs(),
            new Path(metaClient.getMetaAuxiliaryPath()),
            HoodieActiveTimeline.INCLUDED_EXTENSIONS_IN_ACTIVE_TIMELINE);

    List<HoodieInstant> instantsToBeDeleted =
        instants.stream().filter(instant1 -> {
          return instant1.getTimestamp().compareTo(instant.getTimestamp()) <= 0;
        }).collect(Collectors.toList());

    boolean success = true;
    for (HoodieInstant instant1 : instantsToBeDeleted) {
      log.info("Deleting instant " + instant1 + " in auxiliary meta path " + metaClient.getMetaAuxiliaryPath());
      Path metaFile = new Path(metaClient.getMetaAuxiliaryPath(), instant1.getFileName());
