...
public abstract class CompactionStrategy implements Serializable {
  ...
  /**
   * Generate Compaction workload. Allows clients to order and filter the list of compactions to be set.
   * The default implementation takes care of setting compactor Id from configuration allowing subclasses
   * to only worry about ordering and filtering compaction operations
   *
  ...
  public HoodieCompactionWorkload generateCompactionWorkload(HoodieWriteConfig writeConfig,
    ...
    return HoodieCompactionWorkload.newBuilder().setCompactorId(writeConfig.getCompactorId())
