...
public abstract class AbstractHoodieLogRecordScanner {
  ...
  public void scan() {
    try {
      ...
      Set<HoodieLogFile> seenLogFiles = new HashSet<>();
      while (logFormatReaderWrapper.hasNext()) {
        HoodieLogFile logFile = logFormatReaderWrapper.getLogFile();
        log.info("Scanning log file " + logFile);
        seenLogFiles.add(logFile);
        totalLogFiles.set(seenLogFiles.size());
        ...
        switch (r.getBlockType()) {
          case AVRO_DATA_BLOCK:
            ...
            if (isNewInstantBlock(r) && !readBlocksLazily) {
              ...
              processLastFullyReadCommitLogBlocks(currentInstantLogBlocks, seenLogFiles.size());
            }
  ...
  /**
   * Process the set of log blocks belonging to the last commit which is read fully.
   */
  private void processLastFullyReadCommitLogBlocks(Deque<HoodieLogBlock> lastBlocks, int numLogFilesSeen)
      throws Exception {
