...
/**
 * Buffer of Rows to be rendered. This class takes care of ordering
 * rows and limiting before renderer renders it.
 */
public class TableBuffer implements Iterable<List<String>> {
  ...
  // Raw list of rows
  private final List<List<Object>> rawRows;
  ...
  public TableBuffer(TableHeader rowHeader,
  ...
  /**
   * Main API to add row to the buffer
   * @param row Row
   */
  public TableBuffer add(List<Object> row) {
    if (finishedAdding) {
   ...
   * @return
   */
  public TableBuffer addAll(List<List<Object>> rows) {
   ...
   * @return
   */
  public TableBuffer addAllRows(List<String[]> rows) {
    rows.forEach(r -> add(Arrays.asList(r)));
    return this;
  }

  /**
   * API to let the buffer know writing is over and reading is going to start
   */
  public TableBuffer flip() {
   ...
   * @return
   */
  private List<List<Object>> orderRows() {
    return orderingFieldNameOptional.map(orderingColumnName -> {
      final Comparator<Object> comparator = rowHeader.getComparator(orderingColumnName);
      return rawRows.stream().sorted(new Comparator<List<Object>>() {
        @Override
        public int compare(List<Object> row1, List<Object> row2) {

          Object fieldForRow1 = row1.get(rowHeader.indexOf(orderingColumnName));
          Object fieldForRow2 = row2.get(rowHeader.indexOf(orderingColumnName));
          int cmpRawResult = comparator.compare(fieldForRow1, fieldForRow2);
  ...
  private void sortAndLimit() {
    ...
    final int limit = this.limitOptional.map(r -> r).orElse(rawRows.size());
    final List<List<Object>> orderedRows = orderRows();
