...
class RealtimeUnmergedRecordReader extends AbstractRealtimeRecordReader implements
  ...
  private final BufferedIteratorExecutor<ArrayWritable, ArrayWritable, ?> executor;
  ...
  public RealtimeUnmergedRecordReader(HoodieRealtimeFileSplit split, JobConf job,
    ...
    this.executor = new BufferedIteratorExecutor<>(getMaxCompactionMemoryInBytes(), getParallelProducers(), x -> x);
    // Consumer of this record reader
    this.iterator = this.executor.getBuffer().iterator();
    this.logRecordScanner = new HoodieUnMergedLogRecordScanner(
        ...
        record -> {
          ...
          this.executor.getBuffer().insertRecord(aWritable);
        });
  ...
  private BufferProducer<ArrayWritable> getParallelProducers() {
    BufferProducer<ArrayWritable> logReaderProducer = new FunctionBasedBufferProducer<>(buffer -> {
      logRecordScanner.scan();
      return null;
    });
    BufferProducer<ArrayWritable> parquetReaderProducer = new IteratorBasedBufferProducer<>(parquetRecordsIterator);
    return new CompositeBufferProducer<>(parquetReaderProducer, logReaderProducer);
  }
